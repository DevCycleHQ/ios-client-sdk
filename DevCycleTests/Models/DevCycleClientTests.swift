...
class DevCycleClientTest: XCTestCase {
    ...
    func testFlushEventsWithEvalReasons() throws {
        ...
        let client = try! DevCycleClient.builder().user(self.user).sdkKey("my_sdk_key").options(
            options
        ).build(
            onInitialized: nil)
        client.setup(service: service)
        ...
        XCTAssertEqual(variableDefaultedEvents.count, 2)

        let expectedEvaluatedMetadata = [
            "eval": [
                "reason": "TARGETING_MATCH", "details": "Platform AND App Version",
                "target_id": "target_id_1",
            ]
        ]
        XCTAssertEqual(
            variableEvaluatedEvents["bool-var"]?.metaData as! [String: [String: String]],
            expectedEvaluatedMetadata)
        XCTAssertEqual(
            variableEvaluatedEvents["string-var"]?.metaData as! [String: [String: String]],
            expectedEvaluatedMetadata)

        let expectedDefaultedMetadata = [
            "eval": ["reason": "DEFAULT", "details": "User Not Targeted"]
        ]
        XCTAssertEqual(
            variableDefaultedEvents["title_text"]?.metaData as! [String: [String: String?]],
            expectedDefaultedMetadata)
        XCTAssertEqual(
            variableDefaultedEvents["INVALID Variable!"]?.metaData as! [String: [String: String?]],
            expectedDefaultedMetadata)
    ...
    func testIdentifyUserWithInvalidCachedConfigDoesNotReturnError() {
        let expectation = XCTestExpectation(
            description:
                "identifyUser with invalid cached config should not return error and delete the invalid cached config"
        )
        let failedService = MockFailedConnectionService()
        ...
        client.setup(
            ...
            callback: { error in
                ...
                do {
                    ...
                    XCTFail(
                        "identifyUser should throw when the cached config is available but invalid")
                    expectation.fulfill()
                }
            })
