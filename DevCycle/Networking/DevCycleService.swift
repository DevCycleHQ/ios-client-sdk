...
protocol DevCycleServiceProtocol {
    ...
    func getConfig(user: DevCycleUser, enableEdgeDB: Bool, extraParams: RequestParams?) async throws
        -> UserConfig?
    func publishEvents(events: [DevCycleEvent], user: DevCycleUser) async throws
    func saveEntity(user: DevCycleUser) async throws
    func makeRequest(request: URLRequest) async throws -> DataResponse
}
...
class DevCycleService: DevCycleServiceProtocol {
    ...
    func publishEvents(
        ...
        let eventPayload = self.generateEventPayload(
            events, userId, self.config.userConfig?.featureVariationMap)
        guard
            ...
            DispatchQueue.main.async {
                ...
                guard responseDataJson["statusCode"] == nil else {
                    ...
                    if responseDataJson["message"] is [String] {
                        errorResponse = (responseDataJson["message"] as! [String]).joined(
                            separator: ", ")
                    } else {
        ...
        -> UserConfig?
    {
        try await withCheckedThrowingContinuation {
            (continuation: CheckedContinuation<UserConfig?, Error>) in
            getConfig(user: user, enableEdgeDB: enableEdgeDB, extraParams: extraParams) {
                config, error in
                if let error = error {
    ...
    func publishEvents(events: [DevCycleEvent], user: DevCycleUser) async throws {
        try await withCheckedThrowingContinuation {
            ...
            publishEvents(events: events, user: user) { data, response, error in
                if let error = error {
                    continuation.resume(throwing: error)
                } else {
                    continuation.resume()
    ...
    func saveEntity(user: DevCycleUser) async throws {
        try await withCheckedThrowingContinuation {
            (continuation: CheckedContinuation<Void, Error>) in
            saveEntity(user: user) { data, response, error in
                if let error = error {
                    continuation.resume(throwing: error)
                } else {
                    continuation.resume()
    ...
    func makeRequest(request: URLRequest) async throws -> DataResponse {
        try await withCheckedThrowingContinuation {
            (continuation: CheckedContinuation<DataResponse, Error>) in
